...
class DiTBlock(nn.Module):
    ...
    def __call__(
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = mx.split(biases, 6, axis=1)
...
class DiT(nn.Module):
    ...
    def __init__(self, config: TransformerConfig):
        ...
        self.final_layer_scale_shift_table = mx.random.normal((2, config.dim)) / (config.dim**0.5)
    ...
    def __call__(
        ...
        # Final modulation
        shift, scale = mx.split(self.final_layer_scale_shift_table[None] + t[:, None], 2, axis=1)
