    ...
    if sr != target_sr:
        try:
            ...
            try:
                ...
                raise ImportError(
                    "Please install librosa or scipy for resampling: pip install librosa scipy"
                )
...
class SAMAudioProcessor:
    ...
    def _process_anchors(
        ...
        if anchors is None:
            ...
            null_col = mx.full((batch_size, 1), self.ANCHOR_DICT["<null>"], dtype=mx.int32)
            pad_col = mx.full((batch_size, 1), self.ANCHOR_DICT["<pad>"], dtype=mx.int32)
            ...
            for i, anchor_list in enumerate(anchors):
                ...
                for token, start_time, end_time in anchor_list:
                    start_idx = self.wav_to_feature_idx(int(start_time * self.audio_sampling_rate))
                    end_idx = self.wav_to_feature_idx(int(end_time * self.audio_sampling_rate))

                    # Update alignment for this span
                    anchor_idx = len(current)
                    anchor_alignment_np[i, start_idx : min(end_idx, seq_len)] = anchor_idx
            ...
            padded_ids = []
            for ids in all_ids:
                ids_padded = ids + [self.ANCHOR_DICT["<pad>"]] * (max_anchors - len(ids))
...
def save_audio(
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "Please install soundfile or scipy for audio saving: pip install soundfile scipy"
            )
