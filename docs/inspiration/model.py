...
class SinusoidalEmbedding(nn.Module):
    ...
    def __init__(self, dim: int, theta: float = 10000):
        ...
        assert dim % 2 == 0
        half_dim = dim // 2
        inv_freq = mx.exp(-math.log(theta) * mx.arange(half_dim, dtype=mx.float32) / half_dim)
...
class SAMAudio(nn.Module):
    ...
    def separate(
        ...
        with wired_limit(self):
            ...
            ode_step_fn = self._ode_step_euler if method == "euler" else self._ode_step_midpoint
            ...
            target_wavs = self.audio_codec.decode(target_features, chunk_size=ode_decode_chunk_size)
            mx.eval(target_wavs)
            mx.clear_cache()
            ...
            wav_sizes = [self.audio_codec.feature_idx_to_wav_idx(s) for s in sizes.tolist()]
    ...
    def separate_long(
        ...
        for i in tqdm(range(num_chunks), desc="Processing chunks"):
            ...
            if i > 0 and overlap_samples > 0:
                ...
                blended_target = prev_target_tail * fade_out + curr_target_head * fade_in
                blended_residual = prev_residual_tail * fade_out + curr_residual_head * fade_in
    ...
    def from_pretrained(
        ...
        if weights_path:
            ...
            try:
                ...
                pt_weights = torch.load(str(pt_weights_path), map_location="cpu", weights_only=True)
            ...
            warnings.warn(f"Weights not found at {model_path}. Model will have random weights.")
        ...
        if dtype != mx.float32:
            ...
            model.apply_to_modules(lambda k, m: m.update(tree_map(cast_to_dtype, m.parameters())))
...
def _load_weights(model: SAMAudio, weights: dict, strict: bool = False) -> SAMAudio:
    ...
    if 0 < len(missing) < 50:
        ...
        warnings.warn(f"Missing {len(missing)} parameters: {', '.join(sorted(missing)[:10])}...")
...
def _convert_weight_name(name: str) -> str:
    ...
    if result.startswith("audio_codec.encoder.block.0."):
        result = result.replace("audio_codec.encoder.block.0.", "audio_codec.encoder.conv_in.")
    ...
    if result.startswith("audio_codec.encoder.block.5."):
        result = result.replace("audio_codec.encoder.block.5.", "audio_codec.encoder.snake_out.")

    # encoder.block.6.* -> encoder.conv_out.*
    if result.startswith("audio_codec.encoder.block.6."):
        result = result.replace("audio_codec.encoder.block.6.", "audio_codec.encoder.conv_out.")

    # === AUDIO CODEC DECODER MAPPING ===
    # decoder.model.0.* -> decoder.conv_in.*
    if result.startswith("audio_codec.decoder.model.0."):
        result = result.replace("audio_codec.decoder.model.0.", "audio_codec.decoder.conv_in.")
    ...
    if result.startswith("audio_codec.quantizer.in_proj."):
        result = result.replace("audio_codec.quantizer.in_proj.", "audio_codec.quantizer_in_proj.")
